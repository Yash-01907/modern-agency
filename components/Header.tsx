      ...
      <div className="container mx-auto px-6 py-8 flex justify-between items-center">
        ...
        >
          <div className="w-8 h-8 flex flex-col justify-center gap-1.5 items-end">
            <span className={`h-0.5 bg-white transition-all duration-300 ${isOpen ? "rotate-45 translate-y-2 w-8" : "w-8 group-hover:w-6"}`} />
            <span className={`h-0.5 bg-white transition-all duration-300 ${isOpen ? "-rotate-45 -translate-y-2 w-8" : "w-5 group-hover:w-8"}`} />
          </div>
        </button>
              ...
              <nav className="flex flex-col gap-8 text-center">
                {["Work", "About", "Services", "Contact"].map((item, i) => (
                  <motion.a 
                    ...
                    className="text-6xl md:text-8xl font-display font-bold drop-shadow-xl hover:text-gray-400 transition-colors"
